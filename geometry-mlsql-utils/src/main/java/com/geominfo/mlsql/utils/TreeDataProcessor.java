...
public class TreeDataProcessor {
    ...
    public <T> TreeVo<T> flashIdNamePid(TreeVo<T> vo) {
        ...
        Object obj;
        try {
            if (StringUtils.isNotBlank(vo.getIdKey()) || StringUtils.isNotBlank(vo.getNameKey()) || StringUtils.isNotBlank(vo.getPidKey())|| StringUtils.isNotBlank(vo.getTypeKey()) ) {
                Field[] fields = cl.getDeclaredFields();
                if (fields != null && fields.length > 0) {
                    for (Field f : fields) {
                        name = f.getName();
                        if (name.equalsIgnoreCase(vo.getIdKey()) || name.equalsIgnoreCase(vo.getNameKey()) || name.equalsIgnoreCase(vo.getPidKey())||name.equalsIgnoreCase(vo.getTypeKey())) {
                            type = f.getType();
                            methodName = null;
                            ...
                            if (name.equalsIgnoreCase(vo.getIdKey())) {
                                ...
                                vo.setLabel(value);
                                vo.setTitle(value);
                            } else if (name.equalsIgnoreCase(vo.getTypeKey())) {
                                vo.setType(value);
                                vo.setTypeKey(value);
    ...
    public <T> List<TreeVo<T>> getTreeVoList(List<T> list, String idKey, String nameKey, String pidKey, String typeKey) {
        List<TreeVo<T>> rList = null;
        Map<String, TreeVo<T>> tempMap = null;
        List<TreeVo<T>> tempList = null;
        Set<String> ids = new HashSet<String>();
        ...
        if (list != null && list.size() > 0) {
            for (T t : list) {
                ...
                vo = flashIdNamePid(new TreeVo<T>(t, idKey, nameKey, pidKey, typeKey));
                vo.setRoutePids(TreeDataProcessor.getInstance().getRoutePids(vo.getId(), vo.getParentid(), routeMap));
                tempMap.put(vo.getId(), vo);
                tempList.add(vo);
                ids.add(vo.getId());
