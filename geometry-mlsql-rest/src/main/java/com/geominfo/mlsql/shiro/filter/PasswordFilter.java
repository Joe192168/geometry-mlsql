...
public class PasswordFilter extends AccessControlFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (isPasswordTokenGet(request)) {
            ...
            try {
                redisTemplate.opsForValue().set("TOKEN_KEY_"+ IpUtil.getIpFromRequest(WebUtils.toHttp(request)).toUpperCase()+userKey.toUpperCase(),tokenKey,5000, TimeUnit.SECONDS);
                // 动态秘钥response返回给前端
                ...
                Message message = new Message();
//                message.ok(1004,"issued tokenKey fail");
                message.ok(1004,"登录超时，请重新登录!");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);
            }
            return false;
        }
