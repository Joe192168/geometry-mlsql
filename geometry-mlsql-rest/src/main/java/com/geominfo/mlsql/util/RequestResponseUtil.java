...
import com.geominfo.mlsql.support.XssSqlHttpServletRequestWrapper;
        ...
        if (request.getAttribute("body") != null) {
            ...
            try {
                if(request.getInputStream().available()>0){
                    maps = JSON.parseObject(request.getInputStream(),Map.class);
                }else{
                    maps = getRequestParameters(request);
                }
            }catch (IOException e) {
                e.printStackTrace();
            }
