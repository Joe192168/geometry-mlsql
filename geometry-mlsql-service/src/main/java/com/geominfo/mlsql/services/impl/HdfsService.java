...
public class HdfsService {
     ...
     */
    public void uploadFileToHdfs(String srcFile, String dstPath) throws IOException{
     ...
     * @param dstPath HDFS的相对目录路径，比如：/testDir
     */
    public void uploadFileToHdfs(boolean delSrc, boolean overwrite, String srcFile, String dstPath) throws IOException{
        //源文件路径
    ...
    public boolean checkExists(String path) {
        ...
        try {
            ...
            log.error("判断文件或者目录是否在HDFS上面存在失败，path:{}",path,e);
            return false;
        }finally {
            close(fileSystem);
        }
    ...
    public List<Map<String,Object>> listFiles(String path, PathFilter pathFilter){
        ...
        if(checkExists(path)){
            ...
            try {
                ...
                log.error("获取HDFS上面的某个路径下面的所有文件失败，path:{0}",path,e);
            }finally {
                close(fileSystem);
            }
        }
     ...
     */
    public void downloadFileFromHdfs(String srcFile, String dstFile) throws IOException{
        //HDFS文件路径
     ...
     * @return java.util.List<java.util.Map<java.lang.String,java.lang.Object>>
     */
    //public List<Map<String,Object>> getAllDirWithTree(String path, PathFilter pathFilter){
    public List<Object> getAllDirWithTree(String path, PathFilter pathFilter){
        //返回数据
                ...
                if(statuses != null){
                    for(FileStatus status : statuses){
                        ...
                        if (status.isDirectory()) {
                            String str = status.getPath().toString();
                            String[] split = str.split("/", 4);
                            Object allDirWithTree = getAllDirWithTree("/" + split[3], pathFilter);
                            fileMap.put(str,allDirWithTree);
                        }else {
                            result.add(status.getPath().toString());
                            //fileMap.put(status.getPath().toString(),null);
                        }
